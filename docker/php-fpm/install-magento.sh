...
if [ -z "$(ls -A /var/www/magento)" ]; then
  ...
  if [[ "${ELASTICSEARCH_ENGINE}" ]]; then
    install_args+=("--search-engine=$ELASTICSEARCH_ENGINE")
  fi
  if [[ "${ELASTICSEARCH_HOST}" ]]; then
    install_args+=("--elasticsearch-host=$ELASTICSEARCH_HOST")
  fi
